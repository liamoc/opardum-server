...
%include lhs.include
\begin{document}
\title{Haskellpad: Concurrency Control}
\maketitle

\section{Introduction}

Haskellpad makes use of Operational Transforms (see the @OperationalTransforms@ module) in order to 
...
In this module, we define a set of threads that fulfill these three responsibilities.

> module Haskellpad.ConcurrencyControl where
>
...
> incomingTransform d oplist op = mapAccumL t op $ drop d oplist
...
operation list in the process, which should replace the old list in the server
thread for that client. The final transformed operation is then applied to the snapshot
for verification and storage.

\medskip

Next we will define an IO action for updating the snapshot and verification. It uses
functions from |Control.Exception| to catch pattern match failures in the composition
relation (|+>|).
...
> updateSnapshot op doc = evaluate (Just $ doc +> op) `catch` 
>                            \_ -> return Nothing
