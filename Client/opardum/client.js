...
function hideChange (f) {
   ...
   oldLines = bespin.model._lines.slice(0);
   oldText  = bespin.editor.value;
}

function apply(op) {
  d_c = 0;
  var lines = bespin.model._lines;
  var text  = bespin.editor.value;
  var selection = bespin.editor.selection;
  ...
  for (var i in op) {
     if (op[i].type == "Retain") {
        ...
        console.log("foo",foo);
        bespin.editor.replace({start:foo, end:foo},op[i].data,false);        
        lines = bespin.model._lines;
        ...
        bespin.editor.replace({start:foo, end:bar},'',false);        
        lines = bespin.model._lines;
...
//  bespin.setValue(newtext);  
  ...
  console.log(selection.start);
  bespin.editor.selection = selection;
}
...
bespin.editor.textChanged.add(function(or, nr, nt) {
  var e = {oldRange: or, newRange: nr, newText: nt};
  if (suppress || isSingleton(e.oldRange) && isSingleton(e.newRange)) return;

  var lines = bespin.model._lines.slice(0);
  var text  = bespin.editor.value;
  ...
  if (isSingleton(e.oldRange)) {    
     op.push({type:"Insert", data:bespin.editor.getText(e.newRange)});
  } else {
